...
if with_StressGPs
    ...
    [mStres_std,mGsCrd_std] = Stress_std(mNDspS,mNdCrd,vElStd,...
        mLNodS,vElPhz,cDMatx,mPrLod,mGsStd_omgShp,mGsStd_omgDrv);
    ...
    [mStres_enr,mGsCrd_enr] = Stress_enr(mNDspS,mNDspE,mNdCrd,vElEnr,mLNodS,...
        cLNodE,vElPhz,cDMatx,mPrLod,cGsEnr_omgShS,cGsEnr_omgDvS,cGsEnr_omgDvE);
...
if with_JIntegral
    ...
    Ji = IntJ(cCkCrd,mNDspS,mNDspE,R_sif);
...
% NOTE: The absolute value of the stress intesity factors is not correct
% because the values depend on `G` and `kappa` that are not fixed;
% however the ratio `K2/K1` should be correct.

K1_aux=1; K2_aux=0;
K1 = IntM(cCkCrd, mNDspS, mNDspE, R_sif, ...
    G_nearcracktip, kappa, K1_aux, K2_aux);
...
K1_aux=0; K2_aux=1;
K2 = IntM(cCkCrd, mNDspS, mNDspE, R_sif, ...
    G_nearcracktip, kappa, K1_aux, K2_aux);
...
% NOTE: `KII/KI` constant irrespective of `E_prime`.

% Require for consistency:
% (KI^2 + KII^2)/E_prime == J

% 2*KI*KI_aux/E_prime = M(KI_aux)
% 2*KII*KII_aux/E_prime = M(KII_aux)

% get SIF for tip #1
for i = find(K1(:,1))'
    K1(i,1) = K1(i,1) * E_nearcracktip_star / 2;
    K2(i,1) = K2(i,1) * E_nearcracktip_star / 2;
end

% get SIF for tip #2
for i = find(K1(:,2))'
    K1(i,2) = K1(i,2) * E_nearcracktip_star / 2;
    K2(i,2) = K2(i,2) * E_nearcracktip_star / 2;
end
...
%--------------------------------------------------------------------------
% Plot critical load magnitude
%--------------------------------------------------------------------------

if (plot_DissipGlb || save_StateVarb) && exist('G_crt','var')
     
    fig_critical_load = 1001;
    figure(fig_critical_load);
    clf(fig_critical_load);

    h = gcf; hold on;
    set(h,'Color','w');

    h_plot_1 = plot(sState.cracklength_growavg, sState.loadscale_wrt_Gcrt_growavg, ...
                    '-ok','linewidth',1,'markerfacecolor','w','markersize',5);
    
    h_plot_2 = plot(sState.cracklength_posgrow, sState.loadscale_wrt_Gcrt_posgrow, ...
                    '--xr','linewidth',1,'markerfacecolor','r','markersize',5);

    ylabel({'Critical displacement magnitude (mm)','[nominal applied displacement scaled by sqrt(Gc/Gs)]'})
    xlabel('Crack length (mm)')
    title('Critical displacement magnitude based on critical fracture energy release rate')

    legend([h_plot_1,h_plot_2], ...
        'Gs computed by potential energy differencing', ...
        'Gs computed by the J-Integral method')
    
    if save_StateVarb
        saveas(fig_critical_load,[path_savedImg,'plot_CriticalLoadMagnitude'],'fig');
        if ~plot_DissipGlb; close(gcf); pause(1e-6); end
    end
...
%--------------------------------------------------------------------------
% Plot critical potential energy
%--------------------------------------------------------------------------

if (plot_DissipGlb || save_StateVarb) && exist('G_crt','var')
     
    fig_strain_energy = 1002;
    figure(fig_strain_energy);
    clf(fig_strain_energy);

    h = gcf; hold on;
    set(h,'Color','w');

    h_plot_1 = plot(sState.cracklength_growavg, sState.Pi_scaled_wrt_Gcrt_growavg, ...
                    '-ok','linewidth',1,'markerfacecolor','w','markersize',5);
    
    h_plot_2 = plot(sState.cracklength_posgrow, sState.Pi_scaled_wrt_Gcrt_posgrow, ...
                    '--xr','linewidth',1,'markerfacecolor','r','markersize',5);

    ylabel({'Critical strain energy (mJ)','[nominal applied displacement scaled by Gc/Gs]'})
    xlabel('Crack length (mm)')
    title('Critical strain energy based on critical fracture energy release rate')
    ...
    if save_StateVarb
        saveas(fig_strain_energy,[path_savedImg,'plot_CriticalStrainEnergy'],'fig');
        if ~plot_DissipGlb; close(gcf); pause(1e-6); end
    end
